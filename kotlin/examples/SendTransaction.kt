 ...
 *
 * WARNING: This example uses a test mnemonic and sends real transactions
...
fun main() {
    ...
    println("1. Configuring chain connection...")
    ...
    val mnemonic = System.getenv("XION_MNEMONIC") ?: run {
        println("   XION_MNEMONIC environment variable not set")
    ...
    println("3. Checking sender balance...")
    ...
    if (balanceAmount == 0uL) {
        println("   WARNING: Sender has no uxion balance!")
    ...
    if (balanceAmount < 6000u) {
        println("   WARNING: Insufficient balance ($balanceAmount uxion)")
    ...
    println("4. Preparing and sending transaction...")
    ...
    println("5. Waiting for transaction confirmation...")
    println("   Waiting 10 seconds...")
    Thread.sleep(10000)

    // Step 6: Query transaction result
    println("\n6. Querying transaction result...")
    val txResult = try {
