use mob::{ChainConfig, Client, Coin, SessionMetadata, SessionRustSigner, RustSigner};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let main_signer = RustSigner::from_mnemonic(main_mnemonic.to_string(), "xion".to_string(), None)?;
    ...
    println!("=== Step 4: Create Session RustSigner ===");
    let session_signer = SessionRustSigner::new(Arc::new(session_key_signer), metadata)?;
    ...
    let session_from_key = SessionRustSigner::from_private_key(
        private_key,
        "xion".to_string(),
        main_signer.address(),
        7200, // 2 hours
    ...
    let expired_result = SessionRustSigner::new(
